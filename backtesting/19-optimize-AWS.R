...
dataCSV <- getSymbols.av(tickerToTest, api.key = av_api_key(), adjusted = TRUE, env = .GlobalEnv, output.size = "full", auto.assign = FALSE)
desired_range <- (dataCSV)[date_range]
colnames(desired_range)[6] <- "Adjusted"
...
add.signal(strategy = strategy.st,
           name= "sigCrossover", 
           arguments = list(columns = c("Adjusted", "nSlow"), relationship = "gt"),
           label = "filter")

add.signal(strategy.st, name="sigCrossover",
           arguments=list(columns=c("Adjusted", "nSlow"), relationship="lt"),
           label="filterExit")
